...
const speedfont=(venusq2||fenix8||fenixe||fenix7||fr55|mk2)?Gfx.FONT_NUMBER_MEDIUM:Gfx.FONT_NUMBER_HOT;
const glucosefont=((fr735xt||venusq2||fenix8||fenixe)||fr965)?Gfx.FONT_SYSTEM_NUMBER_HOT:Gfx.FONT_SYSTEM_NUMBER_THAI_HOT;
...
const speedy=(mk2||fr55)?(height*.61):(fenix8?height*0.56:(fenix7?(height*.61):( venusq2?(height*.65):((  edge1040||edgeexplore2||edge830)?(height*.48):(fr965?height*.57:(height*.59))))));
const lapsfont=(fenix7||fr55||fr935||fr570|mk2)?Gfx.FONT_LARGE:Gfx.FONT_NUMBER_MEDIUM;
var starty,y;
var     speedunity;
const fh=fromh-height*0.05;
const lapvaluey=
fenix8?height*.54:((venusq2)?(fh+height*.60):((fr570||fr965||fr935||instinct3)?height*.52 :( edgeexplore2?(height*.57): (fromh+height*.57))));
const x=width*.57;
var verh;
var    lapnamey,timey;
var hearty;
function onLayout(dc) {
    ...
    hearty=height-hheight*(fr935?1.1:(mk2?0.8:.88));
    }
...
//            dc.drawLine(xline*.5, yp+glucoserate*height*.05, xline, yp);
    ...
    if(locinfo!=null) {
        ...
        dc.drawText(width*.99,speedunity ,Gfx.FONT_XTINY,initer.speedunits, speedunitJust);
    ...
    if(actinfo!=null) {
        if(actinfo.elapsedDistance!=null) {
            ...
            if(initer.showdistanceunit) {
                dc.drawText(width*.13,ydistunits ,Gfx.FONT_XTINY,initer.distanceunits, Gfx.TEXT_JUSTIFY_RIGHT);
                }
            }
