            ...
            if(fr735xt||instinctE) {
                 timeoff=-theight*0.15;
                    ...
                    if(width>340||fr965) {
                        ...
                        if(fr935) {
                             timeoff=theight*0.22;
                             }
                        else {
                            if(fr55) {
                                 timeoff=theight*0.05;
                                 }
                            }
                        }
                 }
...
function onUpdate(dc) { 
    ...
    if(vers<maxver) {
        var y=yorg;
        var x=width*.615;
         dc.drawText(x,y , glucosefont ,glucosestr,Gfx.TEXT_JUSTIFY_VCENTER| Gfx.TEXT_JUSTIFY_CENTER);
        if(glucoserate==-20) {
...
//        dc.setColor(0xff5112, Gfx.COLOR_TRANSPARENT);
    ...
    var level=load.format("%.0f")+"%";
    if( Toybox.System.Stats has:charging) {
        if(System.getSystemStats().charging) {
            level="+"+level;
            }
       }
    dc.drawText(width*0.975,height*((fr965||fr55)?.37:.35),Gfx.FONT_XTINY,level, Gfx.TEXT_JUSTIFY_VCENTER|Gfx.TEXT_JUSTIFY_RIGHT);
