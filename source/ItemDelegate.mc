...
function switchselect(sel) {
    switch(sel) {
        ...
        case 1: 
                ...
                break;
        case 2: getnum(new dayChange(val),"Day");break;
        case 3: WatchUi.pushView(new MonthView(), new MonthDelegate(val) ,WatchUi.SLIDE_IMMEDIATE) ; break;
        case 4: getnum(new yearChange(val),"Year");break;
        case 5: WatchUi.pushView(new VarView(), new VarDelegate(new varChange(val)) ,WatchUi.SLIDE_IMMEDIATE) ; break;
        case 6: getnum(new datChange(val),vars[val[2]]);break;
        case 7: {
...
if(clickEvent.getType() == CLICK_TYPE_TAP ) { 
 ...
 function onKey(keyEvent) {
     if(alarmactive) {
        ...
        var res=processKey(keyEvent,8,selected);
         if(res<0) {
