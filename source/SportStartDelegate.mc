...
function selectview(id,x) {
    switch(id) {
        case 0: 
            getnum(new lapChange(laplen*toshowdistance),initer.distanceunits+" (0=no)");
            break;
        case 1:    
 ...
 function onKey(evt) {
     if(alarmactive) {
         stopalarm();
          ...
          if(sportStartSel>=0) {
                var res=processKey(evt,3,sportStartSel);
                if(res<0) {    
                        selectview(sportStartSel,width-1);
                        }
                 else {
                        if(subsport<0&&res==2) {
                            if(sportStartSel>res) {
                                sportStartSel=1;
                                }
                            else {
                                sportStartSel=3;
